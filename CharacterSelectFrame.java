...
public class CharacterSelectFrame extends JFrame {
    ...
    // FOR Testing
    private final Color[] colorArray = { new Color(0, 204, 102), new Color(204, 0, 204), Color.WHITE, new Color(204, 204, 0),
            new Color(204, 0, 0), Color.BLUE };
    // FOR Testing
    private final String[] imageNameArray = { "resources/green.png", "resources/plum.png", "resources/white.png",
    ...
    private void requestAvailableCharactersAndUpdate() {
        try {
            // request again all available/unavailable characters
    ...
    private int determinePanelClickedOn(int x, int y) {
        if (x <= 193 && y <= 160) // test for the top left panel
            ...
            return 2;
        else if (x <= 193 && y <= 320) // test for middle left panel
            return 3;
        else if (x >= 194 && y <= 320) // test for middle right panel
            return 4;
        else if (x <= 193) // test for bottom left panel
            return 5;
        ...
        while (!clientConnection.isPlayerConnected()) {
            ...
            switch (ans) {
                case JOptionPane.YES_OPTION:
                    ...
                    String reconnectStr = "Attempting to reconnect on port " + ClueGameConstants.PORT;
                     JOptionPane.showMessageDialog(null, reconnectStr,
    ...
    private void showCharacterUsedandNewAvailableRequest(){
        ...
        btnAdvance.setEnabled(false);
        // request again all available/unavailable characters
        requestAvailableCharactersAndUpdate(); //Internal try/multi-catch
        // update panel with available/unavailable characters
        updatePanelArrayUsedCharacters();
    }
